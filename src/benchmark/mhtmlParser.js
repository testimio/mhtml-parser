...
var DataSection = function (type, filename, location, contentId, charset, encoding, buffer, baseFileName) {
    this.data = {};
    this.data.type = type;
...
DataSection.prototype.buildBufferContent = function (buffer, encoding) {
    ...
    if (encoding.toLowerCase() === "base64") {
        ...
        return buffer.join("");
    } else if (encoding.toLowerCase() === QUOTED_PRINTABLE) { //quoted-printable decoding
...
DataSection.prototype.buildDataPath = function (location, baseFileName) {
    var p = urlParser.parse(decodeURIComponent(location).replace(/\s/g, ''), true);
    var path = "";
    if (p.pathname) {
    ...
    if (p.query && Object.keys(p.query).length > 0) {
        for (var name in p.query) {
            if (p.query[name]) {
                var key = name;
                var value = p.query[name];
                query = "_" + query + key + "_" + value;
            }
        }
    }

    var dataPath = (path + query) || "";
    var dataPathSliceLength = dataPath.length - MAX_FILE_NAME_LENGTH;
...
DataSection.prototype.getDataType = function () {
    var encoding = this.data.encoding.toLowerCase();
    ...
    } else if (contains(type, "font") || contains(location, ".woff") || contains(location, ".woff2") || contains(location, ".ttf") || contains(location, ".eot")) {
        return QUEUE_TYPES.FONT;
    } else if (contains(type, "css")) {
...
var MHTMLParser = function (mhtml, baseFileName) {
    logger.info("Initialized MHTMLParser", {baseFileName: baseFileName});
    this.mhtml = mhtml;
    this.files = [];
...
MHTMLParser.prototype.getBoundary = function () {
    return new Promise((resolve, reject) => {
        var lines = this.mhtml.split("\n");
        lines.forEach((line, index) => {
            line = line.trim();
            if (line.startsWith(BOUNDARY)) {
                var c = '"';
                var boundary = line.substring(line.indexOf(c) + 1, line.lastIndexOf(c) - line.indexOf(c) - 1);
                return resolve({boundary: boundary, line: index + 1});
            }
        });
...
MHTMLParser.prototype.buildDataset = function (boundary, boundaryLine) {
    ...
    return new Promise(resolve => {
        ...
        for (var i = boundaryLine; i < lines.length; i++) {
            var line = lines[i];
            if (contains(line, boundary)) { //Check if this is a new section
                if (buffer && buffer.length > 0) {//If this is a new section and the buffer is full, write to dataset
                    ...
                    var dataSection = new DataSection(type, filename, location, contentId, charset, encoding, buffer, this.baseFileName);
...
MHTMLParser.prototype.getFilePathname = function (location) {
    var urip = urlParser.parse(location);
    var urisearch = urip.search;
    var fullpathname = urip.pathname.split("/");
    var retPath = (fullpathname[fullpathname.length - 2] + "/" || '') + fullpathname[fullpathname.length - 1] + (urisearch || '');
...
MHTMLParser.prototype.replaceFontAndImages = function (data, dataset) {
    var pathname = this.getFilePathname(data.location);
    var re = newRegExp(data.location, "mgi");

    var regExRelativeUrl = newRegExp('url\(([^\)]*' + pathname + '[^\)]*)\)', "g");
    var regExRelativeUrlValue = 'url(\'' + data.path + '\'';

    var regExUrl = newRegExp('url\(([^\)]*' + data.location + '[^\)]*)\)', "g");
    var regExUrlValue = 'url(\'' + data.path + '\'';

    var regExRelativeSrc = newRegExp('src="([^"]*' + pathname + '[^"]*)"', "mgi");
    var regExRelativeSrcValue = 'src="' + data.path + '"';

    for (var i = 0; i < dataset.cssQ.length; i++) {
    ...
    for (var j = 0; j < dataset.framesQ.length; j++) {
...
MHTMLParser.prototype.buildImageQ = function (dataset) {
    ...
    for(let i = 0; i < numOfIterations; i++) {
        ...
        this.files.push({fileName: data.fileName, data: data.data, type: data.type, encoding: "base64"});
    }
...
MHTMLParser.prototype.buildImagesQuotedPrintableQ = function (dataset) {
    ...
    dataset.imagesQuotedPrintableQ.forEach(data => {
        ...
        this.files.push({fileName: data.fileName, data: data.data, type: data.type, encoding: "text"});
    });
    const end = new Date().getTime();
    const time = end - start;
...
MHTMLParser.prototype.buildCssQ = function (dataset) {
    ...
    dataset.cssQ.forEach(data => {
        ...
        var pathname = this.getFilePathname(data.location);
        var re = newRegExp('href="([^"]*' + pathname + '[^"]*)"', "g");
        var cssHref = 'href="' + data.path + '"';
...
MHTMLParser.prototype.buildFrameQ = function (dataset) {
    ...
    const start = new Date().getTime();
    for (var i = 0; i < dataset.framesQ.length; i++) {
        var frameData = dataset.framesQ[i];
        ...
        for (var j = 0; j < dataset.framesQ.length; j++) {
            var frameData2 = dataset.framesQ[j];
            var re = newRegExp("cid:" + frameData2.contentId, "mgi");
...
MHTMLParser.prototype.buildHtml = function (dataset) {
    ...
    this.files.push({fileName: this.baseFileName + ".html", data: body, type: "text/html", encoding: "text"});
